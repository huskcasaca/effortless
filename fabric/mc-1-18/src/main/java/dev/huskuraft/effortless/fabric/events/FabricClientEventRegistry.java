...
public class FabricClientEventRegistry extends ClientEventRegistry {
    ...
    public FabricClientEventRegistry() {
        ...
        ClientShadersEvents.REGISTER.register((manager, sink) -> {
            getRegisterShaderEvent().invoker().onRegisterShader((resource, format, consumer) -> sink.register(new ShaderInstance(manager, resource.getPath(), format.reference()), shaderInstance -> consumer.accept(new MinecraftShader(shaderInstance))));
        });
        ...
        ClientRenderEvents.GUI.register((matrixStack, f) -> {
            getRenderGuiEvent().invoker().onRenderGui(new MinecraftRenderer(matrixStack), f);
        });
