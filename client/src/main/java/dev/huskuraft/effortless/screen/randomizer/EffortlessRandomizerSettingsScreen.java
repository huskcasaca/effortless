...
public class EffortlessRandomizerSettingsScreen extends AbstractScreen {
    ...
    public EffortlessRandomizerSettingsScreen(Entrance entrance, Consumer<RandomizerSettings> consumer, RandomizerSettings randomizerSettings) {
        super(entrance, Text.translate("randomizer.settings.title"));
    ...
    public void onCreate() {
        ...
        this.duplicateButton = addWidget(Button.builder(getEntrance(), Text.translate("randomizer.settings.duplicate"), button -> {
            if (entries.hasSelected()) {
        ...
        this.newButton = addWidget(Button.builder(getEntrance(), Text.translate("randomizer.settings.new"), button -> {
            new EffortlessRandomizerEditScreen(
                    getEntrance(),
                    randomizer -> {
        ...
        this.resetButton = addWidget(Button.builder(getEntrance(), Text.translate("randomizer.settings.reset"), button -> {
            entries.reset(getEntrance().getContentCreator().getDefaultRandomizers());
        }).bounds(getWidth() / 2 - 154, getHeight() - 52, 308, 20).build());

        this.doneButton = addWidget(Button.builder(getEntrance(), Text.translate("randomizer.settings.done"), button -> {
            updateSettings();
            applySettings.accept(lastSettings);
            detach();
        }).bounds(getWidth() / 2 - 154, getHeight() - 28, 150, 20).build());
        this.cancelButton = addWidget(Button.builder(getEntrance(), Text.translate("randomizer.settings.cancel"), button -> {
            detach();
